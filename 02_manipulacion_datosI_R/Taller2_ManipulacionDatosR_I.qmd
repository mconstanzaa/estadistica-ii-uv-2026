...
💾 **Paso 2:** Movemos el archivo **"Script Taller 2 EstadisticaII.R"** a la carpeta `scripts`

<br>

📎 **Paso 3:** Movemos las bases **ELSOC_Wide_2016_2023.RData** y **Base Evaluaciones ELPI III_reducida.RData** a la carpeta `data`\
...
2.  Importar la base de datos `ELSOC_Wide_2016_2023.RData` utilizando la función `load()` en una sección del script llamada `Base de datos`.

3.  Explorar la base de datos utilizando las funciones `dim()`, `names()`, `str()`, `head()` y `tail()` en una sección del script llamada `Exploración base de datos`.
...
::: {style="font-size: 28px;"}
1.  Crea una base de datos llamada `data_elsoc_2023` aplicando los siguientes filtros con `filter()` en una sección del script llamada `Filtros`:
...
4.  Guarda la base de datos editada en formato RData en la carpeta `output` con el nombre `data_elsoc_2023`.
:::
...
✅ Vimos cómo tratar valores perdidos (`NA`) con `rowSums()`, `rowMeans()` y `TRUE ~ NA_character_`
:::
