---
title: "Taller 11: Chi-cuadrado en R"
...
💾 **Paso 2:** Movemos el archivo **"Script Taller 11 EstadisticaII.R"** a la carpeta `scripts`
...
💡 ¡Hoy avanzamos hacia la **inferencia sobre variables categóricas**!

------------------------------------------------------------------------

## Actividades prácticas

A lo largo de la clase realizaremos **dos actividades prácticas**, una por bloque temático.
...
```{r echo=TRUE}
# Actividades Taller 11 Estadística II --------------------------------------------
...
El output entrega tres elementos clave:
...
## Actividad práctica 1 — Chi-cuadrado

::: {style="font-size: 30px;"}
Van a trabajar con la variable `religion_14`: *¿Cuál de estas ideas expresa de mejor modo su juicio respecto del aborto?*. En una sección del script llamada `Chi-cuadrado`:

1.  Recodifica los valores no válidos de `religion_14` con `case_when()` (valores -8 y -9 como `NA_real_`).

2.  Asigna etiquetas con `factor()`: categorías 1 = "Siempre prohibido", 2 = "Solo en casos especiales", 3 = "Opción para las mujeres".

3.  Elimina los NA de `religion_14` con `drop_na()` y verifica el tamaño de la base con `dim()`.

4.  Calcula los descriptivos univariados de `religion_14` y `sexo` con `group_by()` y `summarise()`.

5.  Construye la tabla de contingencia con `sjt.xtab()` y aplica `chisq.test()`. Escribe como comentario (`#`) si se cumplen los supuestos de chi-cuadrado, si existe o no asociación y por qué.
:::
...
## Actividad práctica 2 — Análisis completo

::: {style="font-size: 33px;"}
Con las mismas variables de la Actividad 1 (`religion_14` × `sexo`):

1.  Crea la tabla de contingencia como objeto con `table()`.
...
4.  Redacta como comentario (`#`) una conclusión integrando los tres resultados: $\chi^2$, V de Cramer y residuos.
:::
